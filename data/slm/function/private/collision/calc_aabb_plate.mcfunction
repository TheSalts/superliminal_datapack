...
if score @s AABB.max.z >= @n[tag=temp.plate.AABB,type=block_display] AABB.min.z run return run scoreboard players set @s var 2
execute if entity @n[tag=temp.plate.AABB,type=block_display] run scoreboard players set @s var 0

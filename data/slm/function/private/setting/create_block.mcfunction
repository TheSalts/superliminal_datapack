...
execute as @e[tag=temp,tag=target,type=block_display] run data merge entity @s {transformation: {translation: [-0.5f, -0.5f, -0.5f]}, teleport_duration: 1}
...
execute as @e[tag=temp,tag=interaction,type=interaction] at @s run ride @s mount @e[tag=target,tag=temp,limit=1,sort=nearest,type=block_display]
